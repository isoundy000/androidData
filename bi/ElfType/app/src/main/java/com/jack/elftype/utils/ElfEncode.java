package com.jack.elftype.utils;

import android.util.Log;

import com.jack.elftype.AddSection;
import com.jack.elftype.ElfType32;
import com.jack.elftype.Utils;

/**
 * Created by jackzhous on 17-11-9.
 */
public class ElfEncode {

    public static ElfType32 type_32 = new ElfType32();


    public void startParseSo(){
        byte[] fileByteArys = Utils.readFile("/sdcard/123/libhello-jni.so");
        ...
        //初始化AddSection中的变量
        AddSection.sectionHeaderOffset = Utils.byte2Int(type_32.hdr.e_shoff);     //section header偏移量  0x3130
        AddSection.stringSectionInSectionTableIndex = Utils.byte2Short(type_32.hdr.e_shstrndx);   //section header代表的字节区的索引 0x0014 = 20
        AddSection.stringSectionOffset = Utils.byte2Int(type_32.shdrList.get(AddSection.stringSectionInSectionTableIndex).sh_offset);  //所代表的区段的偏移 0x3750
        // Log.i("jackzhous", "lastLoadVaddr " + Utils.bytes2HexString(type_32.shdrList.get(AddSection.stringSectionInSectionTableIndex).sh_offset) + " " + AddSection.sectionHeaderOffset + " "+ Utils.bytes2HexString(type_32.hdr.e_shstrndx) + AddSection.stringSectionInSectionTableIndex + "  " + " -- " + AddSection.stringSectionOffset);
        ...
        // program offset 0x34  lastLoadInPHIndex  = 2这个的偏移是0x74
        int lastLoadVaddr = Utils.byte2Int(type_32.phdrList.get(AddSection.lastLoadInPHIndex).p_vaddr);  // 0x3eb8 此成员给出段的第一个字节将被放到内存中的虚拟地址
        int lastLoadMem = Utils.byte2Int(type_32.phdrList.get(AddSection.lastLoadInPHIndex).p_memsz);   //0x014c 此成员给出段在内存映像中占用的字节数。可以为 0。
        int lastLoadAlign = Utils.byte2Int(type_32.phdrList.get(AddSection.lastLoadInPHIndex).p_align);  //0x1000
        AddSection.addSectionStartAddr = Utils.align(lastLoadVaddr + lastLoadMem, lastLoadAlign);  //保证添加的sectionHeader字节所代表的内容区域是align对齐的 0x5000
        Log.i("jackzhous", "lastLoadVaddr " + " -- " + AddSection.lastLoadInPHIndex + " -- " + lastLoadMem + " -- " + lastLoadAlign + " -- " + AddSection.addSectionStartAddr);
        System.out.println("start addr hex:"+Utils.bytes2HexString(Utils.int2Byte(AddSection.addSectionStartAddr)));

        /**
         * 添加一个Section
         * 以下的顺序不可乱，不然会出错的
         * 1、添加一个Section Header   ---- 但是为什么可以将section header加到源文件末尾呢？？？？
        ...
        Utils.saveFile("/sdcard/123/libhello-jni1.so", fileByteArys);
    }
