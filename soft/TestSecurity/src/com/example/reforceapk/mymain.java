...
public class mymain {
	...
	public static void main(String[] args) {  
        ...
        try {  
            File payloadSrcFile = new File("force/test.apk");   //需要加壳的程序  
            System.out.println("apk size:"+payloadSrcFile.length());  
            File unShellDexFile = new File("force/test.dex");    //解客dex  
    ...
    private static byte[] encrpt(byte[] srcdata){  
        for(int i = 0;i<srcdata.length;i++){  
            srcdata[i] = (byte)(0xFF ^ srcdata[i]);  			//对源apk数据进行异或操作，解密用异或解密回来即可
        }  
        return srcdata;  
    }  
